...
/** Returns the size of Stella Object in bytes. */
size_t get_stella_object_size(const stella_object* object);

/** Wrapper for each Stella object that contains a custom header. */
typedef struct {
  void*         forwarded;   /**< Pointer to the location where object was moved during "forward" stage */
  stella_object object;     /**< Requested Stella object */
} stella_object_wrapper;

/** Returns the size of Stella Object Wrapper in bytes. */
size_t get_stella_object_wrapper_size(const stella_object_wrapper* object);

/** Returns Stella Object from Stella Object Wrapper. */
stella_object_wrapper* get_object(void* pointer);

/** Structure containing all information about part of memory (from/to). */
typedef struct {
  int   generation; /**< Identificator of generation for garbage collector. */
  int   size;       /**< Allocation size of memory. */
  void  *next;      /**< Pointer to first free byte in space. */
  void* heap;       /**< Pointer to the beginning of allocated piece of memory. */
} space;

/** Initialize Space with requested heap size. */
space initialize_generation_space(int generation, int size);

/** Check if pointer contains in Space. */
bool space_contains(space* space, const void* pointer);

/** Allocate memory in Space with given size in bytes. */
stella_object_wrapper* space_allocate(space* space, const size_t requsted_size);

/** Print Space state for debugging purposes. */
void print_space(const space* space);

/** Structure that containing information about Garbage Collector Generation. */
typedef struct {
  int id;             /**< Generation identifier (0/1). */
  int collect_count;  /**< Number of collections. */
  space* from;        /**< Fresh area of heap where allocated new objects. */
  space* to;          /**< Space where objects are transferred whithin Copy Collection. */
  void* scan;         /**< "scan" variable for Breadth-first coping garbage collection algorithm. */
} generation;

/** Initialize Generation with given "from" and "to" Spaces. */
generation initialize_generation(int id, space *from, space *to);

/** Print Generation state for debugging purposes. */
void print_generation(const generation* g);

/** Chase function in Copying Grabage Collector algorithm. */
bool chase(generation* g, stella_object_wrapper* p);

/** Forward function in Copying Grabage Collector algorithm. */
void* forward(generation* g, void* pointer);

/** Function that forward all root pointers. */
void forward_roots(generation* g);

/** Function that forward object linked fields. */
void forward_object_fields(generation* g, stella_object_wrapper* object);

/** Function that forward objects from previous generations. */
void forward_previous_generation_objects(generation* g);

/** Function that forward fields of changed objects. */
...
/** Function taht handle copying and generation switching logic. */
void handle_generation_switch(generation* g);

/** Forward function in Copying Grabage Collector algorithm. */
void collect(generation* g);

/** Trying to allocates new space. */
